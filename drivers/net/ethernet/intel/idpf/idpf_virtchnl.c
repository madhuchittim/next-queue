...
int idpf_send_create_vport_msg(struct idpf_eth_adapter *adapter)
{
	...
	vport_msg->vport_index = cpu_to_le16(aux_dev->aux_info.id);
	...
	err = idpf_vport_calc_total_qs(adapter, vport_msg, true);
	if (err) {
...
int idpf_vport_alloc_vec_indexes(struct idpf_vport *vport)
{
	...
	vec_info.index = aux_dev->aux_info.id;
...
int idpf_vport_adjust_qs(struct idpf_vport *vport)
{
	...
	err = idpf_vport_calc_total_qs(vport->adapter, &vport_msg, false);
	if (err)
		return err;
